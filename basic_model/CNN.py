...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        tempInput = input_width
        
        for i in range(len(strides)):
            tempInput = (tempInput - kernel_sizes[i] + strides[i]) // strides[i]
            
        self.convolutional_layers = [Conv1D(num_channels[i], num_channels[i+1], kernel_sizes[i], strides[i], conv_weight_init_fn, bias_init_fn) for i in range(self.nlayers)]
        self.flatten = Flatten()
        self.linear_layer = Linear(tempInput * num_channels[-1], num_linear_neurons, linear_weight_init_fn, bias_init_fn)
    ...
    def forward(self, x):
        ...
        for idx in range(len(self.convolutional_layers)):
            self.output = self.convolutional_layers[idx](self.output)
            self.output = self.activations[idx](self.output)
    ...
    def backward(self, labels):
        ...
        for idx in range(len(self.convolutional_layers) - 1, -1, -1):
            grad = grad * self.activations[idx].derivative()
            grad = self.convolutional_layers[idx].backward(grad)
